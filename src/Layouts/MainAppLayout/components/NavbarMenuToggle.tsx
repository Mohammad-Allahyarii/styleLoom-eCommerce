...
const NavbarMenuToggle = ({ isOpen, onToggle, panelId, ref }: PropsType) => {
  return (
    <button
      ...
      className="bg-dark-12 text-absolute-white font-roboto-regular min-w-max rounded-xl relative z-50 py-2.5 px-5 flex justify-center items-center gap-1 cursor-pointer transition-all duration-300 ease-in-out motion-reduce:transition-none text-[16px] lg:text-[16px] aspect-square p-4! md:hidden focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brown-60"
    >
      {isOpen ? (
