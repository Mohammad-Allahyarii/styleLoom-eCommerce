  ...
  return (
    <motion.button
      className={clsx(
        buttonClass,
        className,
        'max-w-max rounded-xl relative py-2.5 px-5 cursor-pointer transition-all duration-300 ease-in-out text-[16px] lg:text-[16px]',
      )}
      whileHover={{ scale: 1.02 }}
      onClick={onClick}
      style={{
        backgroundImage:  variant === 'cornerBordered' || variant === 'bordered' ? dashedBorderImage : 'none'
      }}
    >
      {children}
